...
def generate_label_encoder(df_column: pd.Series) -> LabelEncoder:
    ...
    """
    # generate label encoder using sklearn #
    le = LabelEncoder()
    le.fit(df_column.tolist())
    return le
...
def generate_one_hot_encoder(df_column: pd.Series) -> OneHotEncoder:
    ...
    :return: A label encoder of the column
    """
    # generate oh encoder using sklearn #
    enc = OneHotEncoder()
    return enc.fit(df_column.to_numpy().reshape(-1, 1))
...
def replace_with_label_encoder(df: pd.DataFrame, column: str, le: LabelEncoder) -> pd.DataFrame:
    ...
    """
    # copy just fotr sake of running test script #
    df = df.copy(deep=False)
    df[column] = le.transform(df[column].to_numpy())
    return df
...
def replace_with_one_hot_encoder(df: pd.DataFrame, column: str, ohe: OneHotEncoder, ohe_column_names: List[str]) -> pd.DataFrame:
    ...
    :return: The df with the column replaced with the one from label encoder
    """
    # manually transform columns #
    cols = np.array(ohe.transform(df[column].to_numpy().reshape(-1, 1)).todense().transpose())
    for col, label in zip(cols, ohe_column_names):
        df[label] = col
...
def replace_label_encoder_with_original_column(df: pd.DataFrame, column: str, le: LabelEncoder) -> pd.DataFrame:
    ...
    """
    # use inverse transform to revert #
    df = df.copy(deep=False)
    df[column] = le.inverse_transform(df[column].to_numpy())
    return df
...
def replace_one_hot_encoder_with_original_column(df: pd.DataFrame,
    ...
    """
    # collect all of the one hot encodings column wise
    # revert manual #
    m = []
    for c in columns:
        m.append(df[c].to_numpy())
    df = df.drop(columns=columns)
    # transpose to get proper input shape into row wise format
    m = np.array(m).transpose()
    # replace df col with original values
    df[original_column_name] = ohe.inverse_transform(m)
    return df
